...
void initNoises( void )
{
    ...
    double bigNoiseTolerentToXY = 5.0;
    double gpsAltitudeNoiseScore = 5.0;
...
void pubPath( void )
{
    ...
    // Publish using ROS2 publisher shared pointers (use arrow notation)
    pubOdomAftPGO->publish(odomAftPGO);
    pubPathAftPGO->publish(pathAftPGO);

    static std::unique_ptr<tf2_ros::TransformBroadcaster> br = std::make_unique<tf2_ros::TransformBroadcaster>(nh);
...
void process_pg()
{
    while(1)
    {
		while ( !odometryBuf.empty() && !fullResBuf.empty() && get_imu )
        {
            ...
            // find nearest gps 
            double eps = 0.2; // find a gps topioc arrived within eps second 
            while (!gpsBuf.empty()) {
                ...
                gpsBuf.pop();
                std::cout<<abs(thisGPSTime - timeLaserOdometry)<<std::endl;
            }
            mBuf.unlock(); 
            ...
            if( !gpsOffsetInitialized ) {
                if(hasGPSforThisKF) { // if the very first frame 
                    cout << gpsAltitudeInitOffset << " " << gpsLatitudeInitOffset << " " << gpsLongitudeInitOffset << endl;
            ...
            if( ! gtSAMgraphMade /* prior node */) {
                ...
                mtxPosegraph.lock();
                {
                    ...
                    cout << "odom factor added between node " << prev_node_idx << " and " << curr_node_idx << endl;
    ...
    while(1)
    {
		while ( !scLoopICPBuf.empty() )
        {
            if( scLoopICPBuf.size() > 30 ) {
                ...
                RCLCPP_WARN(rclcpp::get_logger("laser posegraph optimization"), "Too many loop clousre candidates to be ICPed is waiting ... Do process_lcd less frequently (adjust loopClosureFrequency)");
            }
...
int main(int argc, char **argv)
{
    ...
    save_directory = nh->get_parameter("save_directory").get_parameter_value().get<std::string>();
    pgKITTIformat = save_directory + "optimized_poses.txt";
    odomKITTIformat = save_directory + "odom_poses.txt";
    pgTimeSaveStream = std::fstream(save_directory + "times.txt", std::fstream::out); 
    pgTimeSaveStream.precision(std::numeric_limits<double>::max_digits10);
    pgScansDirectory = "/home/raibo-con/ros2_ws/src/SC-PGO/PCD/Scans/";
    ...
    nh->declare_parameter<double>("keyframe_meter_gap", 1.0);
    keyframeMeterGap = nh->get_parameter("keyframe_meter_gap").get_parameter_value().get<double>();
	// nh.param<double>("keyframe_deg_gap", keyframeDegGap, 10.0); // pose assignment every k deg rot 
    nh->declare_parameter<double>("keyframe_deg_gap", 10.0);
    keyframeDegGap = nh->get_parameter("keyframe_deg_gap").get_parameter_value().get<double>();
    keyframeRadGap = deg2rad(keyframeDegGap);

	// nh.param<double>("sc_dist_thres", scDistThres, 0.2);  
    nh->declare_parameter<double>("sc_dist_thres", 1.0);
    scDistThres = nh->get_parameter("sc_dist_thres").get_parameter_value().get<double>();
    ...
    nh->declare_parameter<double>("sc_max_radius", 80.0);
    scMaximumRadius = nh->get_parameter("sc_max_radius").get_parameter_value().get<double>();
    ...
    nh->declare_parameter<double>("mapviz_filter_size", 0.05);
    mapVizFilterSize = nh->get_parameter("mapviz_filter_size").get_parameter_value().get<double>();
    downSizeFilterMapPGO.setLeafSize(mapVizFilterSize, mapVizFilterSize, mapVizFilterSize);

	// ros::Subscriber subLaserCloudFullRes = nh.subscribe<sensor_msgs::msg::PointCloud2>("/velodyne_cloud_registered_local", 100, laserCloudFullResHandler);
    auto subLaserCloudFullRes = nh->create_subscription<sensor_msgs::msg::PointCloud2>("/cloud_registered_body", 100, laserCloudFullResHandler);
	...
	// pubOdomRepubVerifier = nh.advertise<nav_msgs::msg::Odometry>("/repub_odom", 100);
    pubOdomRepubVerifier = nh->create_publisher<nav_msgs::msg::Odometry>("/repub_odom", 100);

	// pubPathAftPGO = nh.advertise<nav_msgs::msg::Path>("/aft_pgo_path", 100);
    pubPathAftPGO = nh->create_publisher<nav_msgs::msg::Path>("/aft_pgo_path", 100);

	// pubMapAftPGO = nh.advertise<sensor_msgs::msg::PointCloud2>("/aft_pgo_map", 100);
    pubMapAftPGO = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/aft_pgo_map", 100);

	// pubLoopScanLocal = nh.advertise<sensor_msgs::msg::PointCloud2>("/loop_scan_local", 100);
    pubLoopScanLocal = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/loop_scan_local", 100);

	// pubLoopSubmapLocal = nh.advertise<sensor_msgs::msg::PointCloud2>("/loop_submap_local", 100);
    pubLoopSubmapLocal = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/loop_submap_local", 100);

    pubOdomGps = nh->create_publisher<nav_msgs::msg::Odometry>("/GpsOdom", 5);
